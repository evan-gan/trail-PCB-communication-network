...
class UIComponent:
    ...
    def __init__(self, container, **kwargs):
        ...
        for key, value in kwargs.items():
            if hasattr(self, key):
                # Instead of setting the attribute directly, store it in the temporary dictionary
                initial_values[key] = value

        # Set all the initial values at once
        for key, value in initial_values.items():
            setattr(self, key, value)

        # Now update the container with the fully initialized component
